//решение Рунге-Кутта через указатель на ф-ию z'=f(z,t)

//f - proisvodnaya v tochke - perpendikulyarna radius vektoru
//=> dvizhenie po okruzhnosti
...
vector<complex<double> > operator*(const vector<complex<double> > &z,
	...
	vector<complex<double> > ret(z);  //тоже самое, что и: ret = z
...
vector<complex<double> > operator+(const vector<complex<double> > &zl,
	...
	if(zl.size() != zr.size()) {
		throw(string("operator+: wrong dimentions"));
	}
	
	vector<complex<double> > ret(zl);  //тоже самое, что и: ret = zl
...
vector<complex<double> > one_step(std::vector<std::complex<double> > (*pf)(const std::vector<std::complex<double> >&, double),const vector<complex<double> > &zp, double dt, double t) {
