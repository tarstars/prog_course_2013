...
void
TestUtil::testMakeTetragonalTensor()
{
...
void
TestUtil::testPolynom() {
    Tensor4 c_ij = makeTetragonalTensor(11, 12, 13, 33, 44, 66);
    Vec3 n(1, 0, 0);
    Matrix G = christoffel(c_ij, n);
    Polynom pol = Matrix2Poly(G);

    QVERIFY(pol.at(3) == -1);
    QVERIFY(pol.at(2) == (G.Get(1,1) + G.Get(2,2) + G.Get(3,3)));
}
